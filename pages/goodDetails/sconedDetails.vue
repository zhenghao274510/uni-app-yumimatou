<template>
	<view class="container">
		...
		<view class="uni-flex uni-space aligItem" style="padding: 10px;">
			<view class="uni-flex aligItem">
				<image src="/static/img/home/xianshimiaosha.png" mode="" class="scond-img"></image>
				<text>限时秒杀</text>
			</view>
			<bbs-count-down :time="LeaveEnd" :setColor="setColor" v-if="v.LeaveEnd!=0"></bbs-count-down>
		</view>
		...
		<view class="introduce-section">
			...
			<view class="uni-flex">
				<view class="price-box">
					<text class="">¥</text>
					<text class="price">{{dataobject.seckillprice}}</text>
					<text v-if="dataobject.units!=''">/{{dataobject.units}}</text>
				</view>
				<view class="sku">
					已售 {{dataobject.salenum}}
				</view>
			</view>
		</view>
		<view class="s-line">
		...
		<view class="uni-flex uni-align-center  c-list">
			...
			<view class="con" v-if="time">19:00～8:00下单的,8:00开始配送,1～2小时送达！</view>
		...
		<!-- 评价 -->
		<eva :list='evaList' @click="moreEva" :productId="productId"></eva>
...
<script>
	...
	import uniGoodsNav from "@/components/second-goods-nav/second-goods-nav.vue"
	...
	export default {
		...
		onShow() {
			...
			this.LeaveEnd = (parseInt(this.endtime) - newTime) * 60 * 60 * 1000 - minTime * 60 * 1000;
			if (h >=19 || h <8) {
				this.Showtime = true
			} else {
				this.Showtime = false
		...
		methods: {
			...
			loadData() {
				let parmas = {
					cmd: 'seckillGoodsdetail',
					seckillid: this.productId
				}
				this._reqw.ipost(parmas).then(res => {
				...
				if (getApp().globalData.uid == '') {
					...
					if (data.salenum >= data.stock) {
						...
						let list = [{
							...
							seckillid: this.productId,
							numbers: 1,
							price: data.seckillprice

						}]
						this.$api.navTo(`/pages/order/finishOrder?type=second&goods=${JSON.stringify(list)}`)
					}
