<template>
	<view class="container">
		<view class="carousel">
			<swiper indicator-dots circular="true" duration="400" autoplay="true" interval='3000' indicator-active-color="#72BD54"
			 indicator-color="#ff55ff">
				<swiper-item class="swiper-item" v-for="(v,k) in dataobject.lazyimages" :key="k">
		...
		<view class="introduce-section">
			<text class="title">{{dataobject.lazyname}}</text>
			<view class="price-box">
				<text class="price-tip">{{dataobject.lazydescs}}{{dataobject.skudesc}}</text>
			</view>
			<view class="uni-flex">
				<view class="price-box">
					<text class="">¥</text>
					<text class="price">{{dataobject.lazyprice}}</text>
		...
		<!-- 评价 -->
		<eva :dataList='evaList' @click="moreEva" :productId="productId"></eva>
...
<script>
	...
	import uniGoodsNav from "@/components/lazy-goods-nav/lazy-goods-nav.vue"
	...
	export default {
		...
		methods: {
			...
			loadData() {
				let parmas = {
					cmd: 'lazyGoodsdetail',
					lazyid: this.productId
				}
				this._reqw.ipost(parmas).then(res => {
					res.result == 0 ? (res.dataobject.lazydescs = this.$api.fromDesc(res.dataobject.lazydescs), this.dataobject =
						res.dataobject, (res.evaluateobj ? this.$set(this.evaList, 0, res.evaluateobj) : ''), this.content =
						this.$api.formText(res.dataobject.lazycontent, uni.getStorageSync(
			...
			buttonClick() {
				if (getApp().globalData.uid == '') {
					...
					let list = [{
						goodsname: data.lazyname,
						goodsimage: data.lazyimage,
						name: data.skudesc,
						lazyid: this.productId,
						numbers: 1,
						price: data.lazyprice

					}]
					this.$api.navTo(`/pages/order/finishOrder?type=lz&goods=${JSON.stringify(list)}`)
				}
			},
			//  底部操作栏
			onClick(e) {
